...
- **Workflow save/load** - serialize graphs as `.bite` JSON files; double-clicking a `.bite` file opens it directly in the app; untrusted params sanitized and version-compatibility checked on load
...
Electron Main Process (Node.js)
  ...
  +-- IPC Handlers    (handlers.ts)
        ^v Electron IPC (contextBridge + ipcMain/ipcRenderer)
...
|   +-- update-utils.ts    - Pure helpers (no electron imports): extractWorkflowPath (find .bite in argv), compareSemver
...
|   |   +-- compatUtils.ts - isCompatible(fileVersion, appVersion): checks a .bite file's version against the app using compat-ranges.json
...
|   |       +-- workflow-sanitize.ts - sanitizeWorkflowGraph: strips `__`-prefixed param keys (e.g. __compute_js__) from untrusted .bite data on load
...
|       |   +-- RunWorkflowDialog.svelte     - Modal listing each output node's readiness (OK/warning + reason); "Run N nodes" / Cancel; imports OutputNodeStatus from RunWorkflowButton.svelte
...
|       |   +-- IncompatibleVersionModal.svelte - Shown when a loaded .bite was saved by an incompatible app version (compatUtils)
...
+-- src/cli/
|   +-- index.ts           - bite entry point (packaged to dist-cli/ via @yao-pkg/pkg); parses named flags, loads the workflow, runs executeBatch per output node
...
+-- workflows/             - User workflow save files (.bite JSON)
+-- scripts/
...
Each workflow node also stores a **`cliName`** param - auto-assigned at creation (e.g. `input-1`, `output-image-1`) and editable in the inspector. This name becomes the CLI flag (`--<cliName>`) in exported shell scripts and is parsed by `bite run`. See section 7.6 CLI Export for details.
...
1. **Topological sort** (Kahn's algorithm) to determine execution order. A cycle (only reachable via a hand-edited/corrupt `.bite`) leaves nodes out of the sort; `topoSort` logs a warning rather than silently dropping them.
...
7. **Thumbnail disk cache** - thumbnails are written as WebP to `%TEMP%/bite-preview/thumb_<hash>_<size>.webp` and reused if the source file has not changed (mtime comparison). At startup, cached thumbnails/previews older than 14 days are pruned (and orphaned `batch_ms_*` intermediates are always removed) so the cache folder doesn't grow without bound.
...
- `IPC.EXPORT_CLI` saves two files side-by-side: a shell script and a companion `.bite` workflow file.
- `bite.exe` is installed alongside the app and added to the user's PATH by the NSIS installer - no separate install step required.
- Supports three shell targets: **PowerShell** (`.ps1`), **Bash** (`.sh`), **CMD** (`.bat`).
- Menu items: File -> Export CLI Script (PS / Bash / CMD).

**Named-flag system:**

Every Input node and output node has a user-editable **CLI Name** param (set in the inspector). The CLI Name becomes the flag name used in the exported script and passed to `bite run`:

```
bite run workflow.bite --input-1 ./photos --input-2 ./overlays --output-image-1 ./out
```
...
- **Execution:** `bite run` parses all `--<flag> <path>` pairs from argv, builds a map from cliName -> path, then iterates every output node in the graph:
...
- File extension: `.bite`. Renamed from `.imgplex` in v0.5.0 when the project became Bite; the file contents are unchanged, so an older workflow can be opened by renaming its extension.
...
- Workflow save / load (`.bite` JSON, dirty-state tracking, close confirmation, untrusted-param sanitization, version-compatibility check with `IncompatibleVersionModal`)
...
- Streaming import with native header parsing (PNG/BMP/WEBP/JPEG/TGA) and batch magick spawning; ~1.6s for ~2000 images (~27x faster than sequential per-image spawning)
- Multi-stream batch pipeline with command fusion (lazy chaining) and single-spawn channel split
- File association - double-clicking a `.bite` file opens the workflow in the app; single-instance enforcement on Windows/Linux brings the existing window to the front for subsequent opens
...
- **CLI named-flag system:**
  ...
  - `bite run` parses `--<flag> <path>` pairs, iterates all output nodes, traces each to its input node, and runs `executeBatch` per output; clear error if a required input flag is missing
...
3. **Additional Platforms** - Bite is currently developed and tested primarily on Windows platform. OS X, Linux and Web versions need additional work to be on parity with Windows.
...
- **WASM-ImageMagick for web "lite" version** - ~3-5x slower than native but viable for single-image preview in a browser.
