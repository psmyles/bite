# Bite Pipeline Test-Workflow Suite
...
paths listed per workflow (e.g. `D:\Dev\bite\test-workflows\out\wf-01`) so
`-AssertOnly` can verify a GUI run. The image lists are **not** stored in `.bite` files -
...
Save each workflow as `test-workflows\wf-NN-<name>.bite` (exact filenames below - the
...
- `fixtures\main` - `alpha_grad_128.png` (white, linear alpha gradient, 128x128),
  `blue_256.png`, `checker_100x80.png` (10 px black/white checker, mean exactly 0.5),
  `gray50_256.png` (solid gray(128)), `green_256.png`, `red_256.png` (solid 256x256)
...
- `fixtures\sets` - `set_alpha_diffuse.png` gray(200), `set_alpha_normal.png` gray(100),
  ...
  `set_beta_rough.png` gray(90) - all 64x64
- `fixtures\flip` - `fb_a_red.png`, `fb_b_green.png`, `fb_c_blue.png` (solid 64x64)

---

## WF-01 - Fast-path chain - `wf-01-fastpath.bite`
...
**CLI:** `bite run wf-01-fastpath.bite --in fixtures\main --out out\wf-01`

**Expected:** 6 output PNGs with the original filenames, all exactly **128x128**.
...
## WF-02 - Light-meta properties -> text report - `wf-02-props-light.bite`
...
**CLI:** `bite run wf-02-props-light.bite --in fixtures\main --report out\wf-02\report.txt`
...
## WF-03 - Heavy-meta properties - `wf-03-props-heavy.bite`
...
**CLI:** `bite run wf-03-props-heavy.bite --in fixtures\meta --report out\wf-03\report.txt`
...
## WF-04 - Channel split -> transform -> merge - `wf-04-channels.bite`
...
**CLI:** `bite run wf-04-channels.bite --in fixtures\main --out out\wf-04`
...
## WF-05 - Analysis-only means + logic gate -> report + images - `wf-05-meanlogic.bite`
...
**CLI:** `bite run wf-05-meanlogic.bite --in fixtures\main --out out\wf-05\images --report out\wf-05\report.txt`
...
## WF-06 - Set-mode texture packing - `wf-06-setmode.bite`
...
**CLI:** `bite run wf-06-setmode.bite --in fixtures\sets --out out\wf-06`
...
## WF-07 - Gate + text filter (conditional file output) - `wf-07-gate.bite`
...
**CLI:** `bite run wf-07-gate.bite --in fixtures\main --out out\wf-07`

**Expected:** exactly one file - `red_256.png`. The other five are suppressed.

---

## WF-08 - All-formats fan-out - `wf-08-formats.bite`
...
```
bite run wf-08-formats.bite --in fixtures\main --out-png out\wf-08\png --out-jpeg out\wf-08\jpeg --out-webp out\wf-08\webp --out-avif out\wf-08\avif --out-tiff out\wf-08\tiff --out-bmp out\wf-08\bmp --out-tga out\wf-08\tga
```
...
## WF-09 - Rename blocks + overwrite semantics - `wf-09-rename.bite`
...
**CLI:** `bite run wf-09-rename.bite --in fixtures\main --out out\wf-09`
...
## WF-10 - Flipbook atlas - `wf-10-flipbook.bite`
...
**Fixtures:** `fixtures\flip` (3 images into a 2x2 grid -> 4th cell shows the background)
...
| Flipbook Output       | rows `2`, cols `2`, cell `64x64`, sort by `name`, output path `...\out\wf-10\atlas.png` |

**CLI:** `bite run wf-10-flipbook.bite --in fixtures\flip --atlas out\wf-10\atlas.png`

**Expected:** `atlas.png` is 128x128; quadrant means: top-left red, top-right green,
bottom-left blue, bottom-right magenta (+/-0.02 per channel).

---

## WF-11 - Math / vector / value plumbing - `wf-11-compute.bite`
...
**CLI:** `bite run wf-11-compute.bite --in fixtures\main --out out\wf-11\images --report out\wf-11\report.txt`
...
- `bite run wf-01-fastpath.bite` **without** `--in` -> non-zero exit, error
...
| format_convert x 7 format definitions                                     | WF-08                                 |
