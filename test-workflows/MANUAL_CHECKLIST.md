...
- [ ] Open `wf-01-fastpath.bite`, load `fixtures\main`, select an image. Click through the
...
- [ ] Open `wf-04-channels.bite` - previews on Split Channels (per-channel grayscale),
      Negate, and Merge Channels (yellow-ish for red input) all render.
- [ ] Open `wf-05-meanlogic.bite` - Mean/Compare/And nodes show live computed values
      on-canvas (`propParams`); selecting `red_256.png` flips And to `true`.
- [ ] Open `wf-06-setmode.bite`, select any `set_alpha_*` file - companion suffixes are
      seeded automatically and the Merge preview shows the packed color.

## Run Workflow dialog & validation

- [ ] In `wf-08-formats.bite`, disconnect one Convert Format -> Image Output wire. Run
      Workflow: the dialog lists that output node as invalid with a reason; the other six
...
- [ ] Point `wf-08-formats.bite` at a folder with many large images (not the fixtures),
...
- [ ] Re-open every `wf-*.bite` fresh: graph, params, and wires intact; no
...
- [ ] File -> Export CLI Script for `wf-01` (PowerShell). Run the exported `.ps1`: it invokes
      `bite run` with `--in`/`--out` flags and produces the same outputs as
