#pragma once
#include <stdint.h>
#include <stddef.h>
#ifdef __cplusplus
extern "C" {
#endif
typedef struct BiteVertex {float pos[2];float uv[2];uint32_t color;} BiteVertex;
typedef struct BiteCommand {uint32_t count;uint32_t index_offset;uint32_t vertex_offset;float clip[4];uint64_t texture;} BiteCommand;
void* bite_create(const char* font_path,float font_size,float ui_scale,const char* ini_path);
void bite_destroy(void* context);
void bite_frame(float width,float height,float scale,float delta);
void bite_render(void);
void bite_mouse_position(float x,float y);
void bite_mouse_button(int button,int down);
void bite_mouse_wheel(float x,float y);
void bite_key(int key,int down);
void bite_text_input(const char* text);
void bite_focus(int focused);
int bite_want_text_input(void);
void bite_font_pixels(const unsigned char** pixels,int* width,int* height);
void bite_font_texture(uint64_t texture);
void bite_set_scale(const char* font_path,float font_size,float ui_scale);
void bite_dockspace(void);
int bite_main_menu_bar_begin(void);
void bite_main_menu_bar_end(void);
int bite_menu_begin(const char* label);
void bite_menu_end(void);
int bite_menu_item(const char* label,const char* shortcut,int selected,int enabled);
void bite_separator(void);
int bite_begin(const char* title);
void bite_end(void);
void bite_text(const char* text);
void bite_panel_header(const char* text);
int bite_button(const char* text);
int bite_selectable(const char* text);
int bite_selectable_drag_source(const char* label,const char* type,const char* payload);
int bite_collapsing_header(const char* label,int default_open);
int bite_drag_float(const char* label,float* value);
int bite_slider_float(const char* label,float* value,float min,float max);
int bite_drag_float_n(const char* label,float* values,int count);
int bite_color_edit4(const char* label,float* values);
int bite_input_text(const char* label,char* data,size_t size);
void bite_set_keyboard_focus_here(void);
int bite_checkbox(const char* label,int* value);
int bite_combo(const char* label,int* current,const char* items);
void bite_next_item_full_width(void);
void bite_begin_disabled(int disabled);
void bite_end_disabled(void);
void bite_image(uint64_t texture,float width,float height);
int bite_image_button(const char* id,uint64_t texture,float width,float height);
int bite_image_button_selected(const char* id,uint64_t texture,float width,float height,int selected);
void bite_same_line(void);
void bite_begin_group(void);
void bite_end_group(void);
void bite_editor_begin(void* context,const char* label);
void bite_editor_end(void);
void bite_node_begin(uint64_t id);
void bite_node_end(void);
void bite_node_header(const char* text,float r,float g,float b);
void bite_node_footer(const char* text);
void bite_typed_pin(uint64_t id,int output,const char* label,float r,float g,float b);
void bite_pin_begin(uint64_t id,int output);
void bite_pin_end(void);
void bite_link(uint64_t id,uint64_t source,uint64_t target,float r,float g,float b);
int bite_new_link(uint64_t* source,uint64_t* target);
int bite_new_node(uint64_t* pin);
int bite_deleted_link(uint64_t* id);
void bite_set_node_position(uint64_t id,float x,float y);
void bite_get_node_position(uint64_t id,float* x,float* y);
void bite_get_node_size(uint64_t id,float* width,float* height);
int bite_node_selected(uint64_t id);
void bite_group(float width,float height);
int bite_background_menu(void);
void bite_canvas_mouse_position(float* x,float* y);
int bite_editor_dragging_selection(void);
int bite_accept_drag_drop_string(const char* type,char* data,size_t size);
void bite_open_popup(const char* id);
int bite_begin_popup(const char* id);
void bite_end_popup(void);
void bite_close_popup(void);
void bite_navigate(void);
int bite_draw_list_count(void);
int bite_vertex_count(int list);
int bite_index_count(int list);
int bite_command_count(int list);
void bite_copy_vertices(int list,BiteVertex* vertices);
void bite_copy_indices(int list,uint32_t* indices);
int bite_get_command(int list,int index,BiteCommand* command);
#ifdef __cplusplus
}
#endif
